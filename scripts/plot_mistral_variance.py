...
Expects two (or more) inference runs under data/parquet/inference/runs/ whose
experiment_id shares a common prefix (default: real-mistral-large3-20).

Outputs to artifacts/mistral_large3_20/:
...
OUT_DIR = REPO_ROOT / "artifacts" / "mistral_large3_20"
...
def run_comparison(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (run_id, exp), g in df.groupby(["run_id", "experiment_id"], sort=True):
        ...
        rows.append({
            ...
            "api_error_rate": (g["failure_mode"] == "api_error").mean(),
        })
...
def chart_per_task_accuracy(plt, stats: pd.DataFrame, out: Path) -> None:
    ...
    style_ax(
        ...
        f"Grey bars = never evaluable (api_error only). "
...
def main() -> None:
    ...
    parser.add_argument("--prefix", default="real-mistral-large3-20",
                        help="experiment_id prefix shared by replicate runs")
