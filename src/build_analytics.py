...
def render_report(df: pd.DataFrame, by_model: pd.DataFrame,
    ...
    sections += [
        ...
        "",
        "```bash",
        "python scripts/demo_bundle.py                 # offline one-command demo (this report)",
        "",
        "# or step by step:",
        "python scripts/fetch_arc_data.py --sample     # 6 offline demo tasks",
        "python scripts/fetch_arc_data.py --limit 20   # optional: + 20 real tasks (needs network)",
        "python src/main.py",
        "python src/run_inference.py --provider mock --model baseline --experiment-id <label>",
        "python src/run_inference.py --provider mock --model mock-large --experiment-id <label>",
        "python src/build_analytics.py --experiment-id <label>",
        "```",
        "",
        "Mock results are deterministic per (model, task): reruns reproduce "
        "these metrics exactly; only run ids and timestamps change.",
        "",
